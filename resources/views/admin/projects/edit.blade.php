...
<div class="app-page-title">
    <div class="page-title-wrapper">
        <div class="page-title-heading">
            ...
            <div>Edit Trip - {{ $project->project_number }}</div>
        </div>
...
<div class="main-card mb-3 card border-warning" id="pending-payments-card">
    ...
    <div class="card-body py-2 d-flex flex-wrap align-items-center gap-4">
        @if($project->pending_to_receive > 0)
        <span class="text-danger"><i class="bi bi-arrow-down-circle me-1"></i> To Receive: <strong>₹{{ number_format($project->pending_to_receive, 2) }}</strong></span>
        @endif
        @if($project->pending_to_give > 0)
        <span class="text-primary"><i class="bi bi-arrow-up-circle me-1"></i> To Pay (Vendors): <strong>₹{{ number_format($project->pending_to_give, 2) }}</strong></span>
        @endif
        @if($project->service_pending > 0)
        <span class="text-warning-emphasis"><i class="bi bi-tools me-1"></i> To Pay (Services): <strong>₹{{ number_format($project->service_pending, 2) }}</strong></span>
        @endif
...
<form action="{{ route('admin.projects.update', $project) }}" method="POST" class="needs-validation" enctype="multipart/form-data" novalidate id="projectForm">
    @csrf
    @method('PUT')
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $project->name) }}" required minlength="2" maxlength="40">
            ...
            <div class="row">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('customer_id') is-invalid @enderror" id="customer_id" name="customer_id" required>
                            ...
                            @foreach($customers as $customer)
                                <option value="{{ $customer->id }}" {{ old('customer_id', $project->customer_id) == $customer->id ? 'selected' : '' }}>{{ $customer->name }} - {{ $customer->mobile }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @php
                            $selectedWorkTypes = old('work_type', $project->work_type ?? []);
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="expected_end_date" name="expected_end_date" value="{{ old('expected_end_date', $project->expected_end_date?->format('Y-m-d')) }}">
                    </div>
                </div>
            </div>
            <div class="mb-3">
                <label for="site_address" class="form-label">Site Address</label>
                <textarea class="form-control" id="site_address" name="site_address" rows="2" maxlength="150">{{ old('site_address', $project->site_address) }}</textarea>
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Trip Description</label>
                <textarea class="form-control" id="description" name="description" rows="2" maxlength="2000">{{ old('description', $project->description) }}</textarea>
            </div>
            <div class="mb-3">
                <label for="project_files" class="form-label">Add More Files</label>
                <input type="file" class="form-control" id="project_files" name="project_files[]" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.gif,.dwg,.dxf">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <div class="input-group has-validation">
                                    ...
                                    <input type="number" class="form-control @error('budget') is-invalid @enderror" id="budget" name="budget" value="{{ old('budget', (int)$project->budget) }}" min="0" max="999999999" step="1" required inputmode="numeric">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">₹</span>
                                    <input type="number" class="form-control" id="advance_received" name="advance_received" value="{{ old('advance_received', (int)$project->advance_received) }}" min="0" max="999999999" step="1" inputmode="numeric">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="mb-0">
                        ...
                        <select class="form-select" id="quotation_id" name="quotation_id">
                            ...
                            @foreach($quotations as $quotation)
                                <option value="{{ $quotation->id }}" {{ old('quotation_id', $project->quotation_id) == $quotation->id ? 'selected' : '' }}>{{ $quotation->quotation_number }} - {{ $quotation->customer->name ?? 'N/A' }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="main-card mb-3 card">
                ...
                <div class="card-body">
                    <div class="row">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('company_id') is-invalid @enderror" id="company_id" name="company_id">
                                    ...
                                    @foreach($companies as $company)
                                        <option value="{{ $company->id }}" data-has-gst="{{ !empty($company->gst_number) ? '1' : '0' }}" {{ old('company_id', $project->company_id) == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                @php
                                    $selectedStaffIds = old('assigned_staff_ids', $project->assigned_staff_ids ?? []);
                    ...
                    <div class="mb-3">
                        ...
                        @php
                            $selectedVendorIds = old('assigned_vendor_ids', $project->assigned_vendor_ids ?? []);
                    ...
                    @include('admin.projects._gst_section', ['gstRates' => $gstRates, 'project' => $project])
                </div>
            </div>
        </div>
    </div>

    @include('admin.projects._services_section', ['existingServices' => $project->projectServices, 'services' => $services, 'banks' => $banks])
