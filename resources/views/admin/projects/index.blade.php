...
<div class="app-page-title">
    <div class="page-title-wrapper">
        ...
        <div class="page-title-actions">
            ...
            <a href="{{ route('admin.projects.create') }}" class="btn btn-primary" title="Create Trip">
...
<div class="main-card mb-3 card">
    <div class="card-header has-tabs d-flex justify-content-between align-items-center">
        <ul class="nav lead-tabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link {{ request('tab') != 'deleted' ? 'active' : '' }}" href="{{ route('admin.projects.index') }}">
                    <i class="bi bi-kanban me-1"></i> Active ({{ $allCount }})
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ request('tab') == 'deleted' ? 'active' : '' }}" href="{{ route('admin.projects.index', ['tab' => 'deleted']) }}">
        ...
        <div class="d-flex align-items-center gap-2">
            ...
            @endif
            <form method="GET" action="{{ route('admin.projects.index') }}" class="d-flex align-items-center gap-2">
                @if(request('tab'))
                ...
                @if(request('search'))
                <a href="{{ route('admin.projects.index', request('tab') ? ['tab' => request('tab')] : []) }}" class="btn btn-sm btn-danger" title="Clear Filters">
            ...
            <thead>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.projects.show', $project) }}"><strong class="name-truncate" title="{{ $project->name }}">{{ $project->name }}</strong></a>
                            <br><small class="text-muted">{{ $project->start_date ? $project->start_date->format('d-m-Y') : '-' }}</small>
                        </td>
                        <td>
                            @if($project->customer)
                            <div>
                                <a href="{{ route('admin.customers.show', $project->customer) }}" class="name-truncate" title="{{ $project->customer->name }}"><strong>{{ $project->customer->name }}</strong></a>
                                <br><small class="text-muted">{{ $project->customer->mobile }}</small>
                                <br><small class="text-muted">Company: {{ $project->company->name ?? '-' }}</small>
                            </div>
                            @else
                                <small class="text-muted">Company: {{ $project->company->name ?? '-' }}</small>
                            @endif
                        </td>
                        <td>{{ formatMoney($project->budget) }}</td>
                        <td class="{{ $project->add_on_total > 0 ? 'text-info' : 'text-muted' }}">{{ formatMoney($project->add_on_total) }}</td>
                        <td class="text-danger">{{ formatMoney($project->total_spent) }}</td>
                        <td class="text-success">{{ formatMoney($project->total_income) }}</td>
                        <td class="{{ $project->profit >= 0 ? 'text-success' : 'text-danger' }}"><strong>{{ formatMoney($project->profit) }}</strong></td>
                        <td>
                            @php
                                $statusColors = [
                        ...
                        <td>
                            <div class="btn-group-actions">
                                ...
                                @else
                                    ...
                                    @if(auth()->user()->hasPermission('projects', 'delete'))
                                    <form action="{{ route('admin.projects.destroy', $project) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this trip?')">
                                        @csrf
                                        @method('DELETE')
