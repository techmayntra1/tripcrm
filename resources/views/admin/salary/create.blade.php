    ...
    <div class="page-title-wrapper">
        <div class="page-title-heading">
            <div class="page-title-icon">
                <i class="bi bi-cash-stack icon-gradient bg-midnight-bloom"></i>
            </div>
            <div>
                Add Salary
                <div class="page-title-subheading">Record a staff salary payment</div>
            </div>
        </div>
...
<div class="main-card mb-3 card">
    <div class="card-header">
        <i class="bi bi-person-badge me-2"></i> Select Staff
    </div>
    <div class="card-body">
        <form method="GET" action="{{ route('admin.salary.create') }}" class="row g-2 align-items-end">
            <div class="col-md-6">
                ...
                <select name="staff_id" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="{{ $s->id }}" {{ ($staff && $staff->id == $s->id) ? 'selected' : '' }}>{{ $s->name }}@if($s->position_name) - {{ $s->position_name }}@endif</option>
                    @endforeach
                </select>
            </div>
        </form>
...
<form action="{{ route('admin.staff.salary-payments.store', $staff) }}" method="POST" id="salaryForm" novalidate>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="main-card mb-3 card">
                <div class="card-header">
                    <i class="bi bi-receipt me-2"></i> Salary Details — {{ $staff->name }}
                </div>
                <div class="card-body">
                    <div class="row mb-3">
                        <div class="col-md-6">
...
<div class="main-card mb-3 card">
    <div class="card-body text-center text-muted py-5">
        ...
        Select a staff member above to record their salary payment.
    </div>
</div>
@endif
@endsection
