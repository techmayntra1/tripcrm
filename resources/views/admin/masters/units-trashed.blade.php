...
<div class="main-card mb-3 card">
    <div class="card-header has-tabs d-flex justify-content-between align-items-center">
        <ul class="nav lead-tabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link" href="{{ route('admin.masters.units') }}">
                    <i class="bi bi-rulers me-1"></i> Active ({{ $activeCount }})
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('admin.masters.units.trashed') }}">
                    <i class="bi bi-archive me-1"></i> Deleted ({{ $items->count() }})
                </a>
            </li>
        </ul>
        <form method="GET" action="{{ route('admin.masters.units.trashed') }}" class="d-flex align-items-center gap-2">
            <div class="input-group input-group-sm" style="width: 250px;">
                <input type="text" class="form-control" name="search" placeholder="Search units..." value="{{ request('search') }}">
                <button class="btn btn-outline-secondary" type="submit"><i class="bi bi-search"></i></button>
            </div>
            @if(request('search'))
            <a href="{{ route('admin.masters.units.trashed') }}" class="btn btn-sm btn-danger" title="Clear"><i class="bi bi-x-lg"></i></a>
            @endif
        </form>
    </div>
    <div class="card-body">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <form action="{{ route('admin.masters.units.toggle', $item) }}" method="POST" class="d-inline">
                            @csrf
