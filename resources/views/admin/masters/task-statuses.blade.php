    ...
    <div class="page-title-wrapper">
        <div class="page-title-heading">
            <div class="page-title-icon">
                <i class="bi bi-list-task icon-gradient bg-warning"></i>
            </div>
            <div>Task Statuses</div>
        </div>
...
<div class="main-card mb-3 card">
    <div class="card-header has-tabs d-flex justify-content-between align-items-center">
        <ul class="nav lead-tabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('admin.masters.task-statuses') }}">
                    <i class="bi bi-list-task me-1"></i> Active ({{ $items->count() }})
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="{{ route('admin.masters.task-statuses.trashed') }}">
                    <i class="bi bi-archive me-1"></i> Deleted ({{ $inactiveCount }})
                </a>
            </li>
        </ul>
        <form method="GET" action="{{ route('admin.masters.task-statuses') }}" class="d-flex align-items-center gap-2">
            ...
            @if(request('search'))
            <a href="{{ route('admin.masters.task-statuses') }}" class="btn btn-sm btn-danger" title="Clear"><i class="bi bi-x-lg"></i></a>
            @endif
        </form>
    </div>
    <div class="card-body">
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <div class="d-flex gap-1 justify-content-center">
                            ...
                            <form action="{{ route('admin.masters.task-statuses.toggle', $item) }}" method="POST" class="d-inline">
                                @csrf
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <h5 class="modal-title"><i class="bi bi-plus-lg me-2"></i>Add Task Status</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('admin.masters.task-statuses.store') }}" method="POST">
                @csrf
...
<div class="modal fade" id="editModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-warning">
                <h5 class="modal-title"><i class="bi bi-pencil me-2"></i>Edit Task Status</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
function openEditModal(item) {
    document.getElementById('editForm').action = '/admin/masters/task-statuses/' + item.id;
