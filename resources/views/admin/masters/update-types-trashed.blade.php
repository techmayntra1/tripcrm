@extends('layouts.app')
@section('title', 'Deleted Update Types')
@section('content')
...
<div class="main-card mb-3 card">
    <div class="card-header has-tabs d-flex justify-content-between align-items-center">
        <ul class="nav lead-tabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link" href="{{ route('admin.masters.update-types') }}">
                    <i class="bi bi-pencil-square me-1"></i> Active ({{ $activeCount }})
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('admin.masters.update-types.trashed') }}">
                    <i class="bi bi-archive me-1"></i> Deleted ({{ $items->count() }})
                </a>
            </li>
        </ul>
        <form method="GET" action="{{ route('admin.masters.update-types.trashed') }}" class="d-flex align-items-center gap-2">
            ...
            @if(request('search'))
            <a href="{{ route('admin.masters.update-types.trashed') }}" class="btn btn-sm btn-danger" title="Clear"><i class="bi bi-x-lg"></i></a>
            @endif
        </form>
    </div>
    <div class="card-body">
