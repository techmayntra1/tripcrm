@extends('layouts.app')
@section('title', 'Deleted ' . $config['label'])
@section('content')
...
<div class="main-card mb-3 card">
    <div class="card-header has-tabs d-flex justify-content-between align-items-center">
        <ul class="nav lead-tabs" role="tablist">
            <li class="nav-item">
                <a class="nav-link" href="{{ route('admin.masters.term-templates', $config['slug']) }}">
                    <i class="bi {{ $config['icon'] }} me-1"></i> Active ({{ $activeCount }})
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link active" href="{{ route('admin.masters.term-templates.trashed', $config['slug']) }}">
                    <i class="bi bi-archive me-1"></i> Deleted ({{ $items->count() }})
                </a>
            </li>
        </ul>
        <form method="GET" action="{{ route('admin.masters.term-templates.trashed', $config['slug']) }}" class="d-flex align-items-center gap-2">
            ...
            @if(request('search'))
            <a href="{{ route('admin.masters.term-templates.trashed', $config['slug']) }}" class="btn btn-sm btn-danger" title="Clear"><i class="bi bi-x-lg"></i></a>
            @endif
        </form>
    </div>
    <div class="card-body">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="4" class="text-center py-4 text-muted">
                        ...
                        No deleted {{ strtolower($config['label']) }}.
                    </td>
                </tr>
                @endforelse
            </tbody>
