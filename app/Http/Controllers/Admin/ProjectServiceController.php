...
use App\Models\ProjectServiceAddon;
...
class ProjectServiceController extends Controller
{
    public function store(Request $request, Project $project)
    {
        ...
        if ($advance > 0 && $bankId) {
            self::createServiceExpense($projectService, $bankId, $advance, now()->toDateString(), 'Service Advance');
        }

        return redirect()->route('admin.projects.show', ['project' => $project, 'tab' => 'services'])
            ->with('success', 'Service added successfully.');
    }

    public function update(Request $request, Project $project, ProjectService $service)
    {
        $validated = $request->validate([
            'service_ids' => 'required|array|min:1',
            'service_ids.*' => 'exists:services,id',
    ...
    public function addPayment(Request $request, Project $project, ProjectService $service)
    {
        $validated = $request->validate([
     ...
     * The expense carries both project_id and project_service_id so it counts
     * once toward project spend and toward the service's paid amount.
     */
    protected static function createServiceExpense(ProjectService $service, $bankId, float $amount, string $date, string $description): Expense
    {
    ...
    public function addAddon(Request $request, Project $project, ProjectService $service)
    {
        $validated = $request->validate([
    ...
    public function deleteAddon(Project $project, ProjectService $service, ProjectServiceAddon $addon)
    {
        $addon->delete();
