...
class TripController extends Controller
{
    ...
    {
        $query = Trip::with(['customer', 'company']);
    ...
    public function create()
    {
        ...
        return view('admin.trips.create', compact('companies', 'customers', 'workTypes', 'quotations', 'staff', 'vendors', 'services', 'banks', 'gstRates'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $trip = DB::transaction(function () use ($validated, $servicesPayload) {
            $trip = Trip::create($validated);
            foreach ($servicesPayload as $svc) {
                $this->createTripService($trip, $svc);
            }
            return $trip;
        });
        ...
        if ($request->hasFile('trip_files')) {
            foreach ($request->file('trip_files') as $file) {
                ...
                $filePath = $file->storeAs('trips/' . $trip->id, $fileName, 'public');
    ...
    {
        $trip->load(['customer', 'company', 'quotation', 'expenses.category', 'expenses.vendor', 'incomes', 'invoices', 'addons', 'files', 'tripServices.addons', 'tripServices.serviceExpenses.bank']);

        $tasks = Task::where('trip_id', $trip->id)
        ...
        return view('admin.trips.show', compact('trip', 'tasks', 'banks', 'services'));
    }

    public function edit(Trip $trip)
    {
        $trip->load(['files', 'tripServices.addons', 'tripServices.serviceExpenses.bank']);
        $companies = Company::orderBy('name')->get();
        $customers = Customer::orderBy('name')->get();
        $workTypes = WorkType::active()->ordered()->get();
        ...
        return view('admin.trips.edit', compact('trip', 'companies', 'customers', 'workTypes', 'quotations', 'staff', 'vendors', 'services', 'banks', 'gstRates'));
    }
    ...
    public function update(Request $request, Trip $trip)
    {
        $validated = $request->validate([
            ...
            'notes' => 'nullable|string|max:150',
            'services' => 'nullable|array',
            'services.*.id' => 'nullable|exists:trip_services,id',
            'services.*.service_ids' => 'required|array|min:1',
            'services.*.service_ids.*' => 'exists:services,id',
            'services.*.amount' => 'required|numeric|min:1',
            'services.*.advance' => 'nullable|numeric|min:0',
        ...
        try {
            DB::transaction(function () use ($trip, $validated, $servicesPayload, $markingCompleted) {
                $trip->update($validated);
                $this->syncTripServices($trip, $servicesPayload);

                if ($markingCompleted) {
                    $trip->refresh()->load('tripServices');
    ...
    public function restore($id)
    {
        ...
        $trip->restore();
        return redirect()->route('admin.trips.index')->with('success', 'Trip restored successfully.');
    }

    public function deleteFile(TripFile $tripFile)
    {
        Storage::disk('public')->delete($tripFile->file_path);
        $tripFile->delete();
    ...
    protected function syncTripServices(Trip $trip, array $servicesPayload): void
    {
        $submittedIds = [];
        foreach ($servicesPayload as $svc) {
            if (!empty($svc['id'])) {
                $row = TripService::where('trip_id', $trip->id)->find($svc['id']);
                if ($row) {
                    $serviceIds = array_values(array_unique(array_map('intval', $svc['service_ids'] ?? [])));
                    $row->update([
                        'service_ids' => $serviceIds,
                ...
                $created = $this->createTripService($trip, $svc);
                $submittedIds[] = $created->id;
            }
        }

        $toDelete = $trip->tripServices()->whereNotIn('id', $submittedIds)->get();
    ...
    public function recordExpensePayment(Request $request, Trip $trip, Expense $expense)
    {
        ...
        return redirect()->route('admin.trips.show', $trip)->with('success', 'Payment of ' . formatMoney($paymentAmount) . ' recorded successfully.');
    }
}
