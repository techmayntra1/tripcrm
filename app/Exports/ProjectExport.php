...
class ProjectExport implements WithMultipleSheets
{
    ...
    public function sheets(): array
    {
        $project = $this->project->load([
            ...
            'projectServices.addons', 'projectServices.serviceExpenses.bank',
            'addons',
        ...
        $sheets[] = new ProjectSheet('Summary', $this->summaryRows($project));

        if ($project->projectServices->count() > 0) {
            $sheets[] = new ProjectSheet('Services', $this->serviceRows($project));
            $paymentRows = $this->servicePaymentRows($project);
            if (count($paymentRows) > 1) {
                $sheets[] = new ProjectSheet('Service Payments', $paymentRows);
            }
        }

        $addonRows = $this->addonRows($project);
        if (count($addonRows) > 1) {
            $sheets[] = new ProjectSheet('Add-Ons', $addonRows);
        }
        ...
        foreach ($expenseTypes as $type => $label) {
            $rows = $this->expenseRows($project, $type);
            if (count($rows) > 1) {
                $sheets[] = new ProjectSheet($label, $rows);
            }
        }

        if ($project->incomes->count() > 0) {
            $sheets[] = new ProjectSheet('Income', $this->incomeRows($project));
        }

        if ($project->invoices->count() > 0) {
            $sheets[] = new ProjectSheet('Invoices', $this->invoiceRows($project));
        }

        $taskRows = $this->taskRows($project);
        if (count($taskRows) > 1) {
            $sheets[] = new ProjectSheet('Tasks', $taskRows);
        }

        if ($project->files->count() > 0) {
            $sheets[] = new ProjectSheet('Files', $this->fileRows($project));
        }

        return $sheets;
    }

    protected function fileRows(Project $project): array
    {
        $rows = [
    ...
    protected function summaryRows(Project $project): array
    {
        $staffNames = $this->namesOf(Staff::class, $project->assigned_staff_ids ?? []);
        $vendorNames = $this->namesOf(Vendor::class, $project->assigned_vendor_ids ?? []);
        $workTypes = is_array($project->work_type) ? implode(', ', $project->work_type) : ($project->work_type ?? '-');

        // Use the same accessors as the on-screen project page so the export and
        // the screen always agree.
        $totalIncome = (float) $project->total_income;
        $totalExpense = (float) $project->total_spent;
        $pendingToReceive = max(0, (float) $project->total_budget - $totalIncome);
        ...
        return [
            ...
            ['Customer Mobile', $project->customer->mobile ?? '-'],
            ['Customer Email', $project->customer->email ?? '-'],
            ['Company', $project->company->name ?? '-'],
            ['Quotation', $project->quotation->quotation_number ?? '-'],
            [],
            ['Start Date', optional($project->start_date)->format('d-m-Y') ?: '-'],
            ['Expected End Date', optional($project->expected_end_date)->format('d-m-Y') ?: '-'],
            ['Actual End Date', optional($project->actual_end_date)->format('d-m-Y') ?: '-'],
            [],
            ...
            ['Add-Ons', (float) $project->add_on_total],
            ['Total Project Value', (float) $project->total_budget],
            ['Total Income', $totalIncome],
            ['Total Expenses (Paid)', $totalExpense],
            ['Profit / Loss', $totalIncome - $totalExpense],
            ['Pending to Receive', $pendingToReceive],
            ['Pending to Give', (float) $project->pending_to_give],
        ];
    }

    protected function serviceRows(Project $project): array
    {
        $rows = [
        ...
        if ($project->projectServices->count() > 0) {
            $rows[] = [
                'TOTAL', '',
                (float) $project->projectServices->sum('amount'),
                (float) $project->projectServices->sum('advance'),
                '',
                (float) $project->projectServices->sum(fn($s) => $s->addons_total),
                (float) $project->projectServices->sum(fn($s) => $s->total_amount),
                (float) $project->projectServices->sum(fn($s) => $s->paid_amount),
                (float) $project->projectServices->sum(fn($s) => $s->balance),
                '', '', '',
            ];
        }
        return $rows;
    ...
    protected function servicePaymentRows(Project $project): array
    {
        $rows = [
    ...
    protected function addonRows(Project $project): array
    {
        $rows = [
    ...
    protected function expenseRows(Project $project, string $type): array
    {
        $rows = [
        ...
        $expenses = $project->expenses->where('expense_type', $type)->sortByDesc('expense_date');
    ...
    protected function incomeRows(Project $project): array
    {
        $rows = [
            ['SR', 'Date', 'Receipt #', 'Type', 'Customer', 'Invoice #', 'Payment Mode', 'Bank', 'Cheque #', 'Cheque Date', 'Description', 'Amount'],
        ];
        $i = 1;
        $incomes = $project->incomes->sortByDesc('income_date');
    ...
    protected function taskRows(Project $project): array
    {
        $rows = [
            ['SR', 'Title', 'Assignee', 'Assignee Type', 'Start', 'Due', 'Status', 'Location', 'Description'],
        ];
        $tasks = Task::with('status')->where('project_id', $project->id)->orderBy('start_at', 'desc')->get();
...
class ProjectSheet implements FromArray, WithTitle, WithEvents
{
