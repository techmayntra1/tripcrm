...
class Project extends Model
{
    ...
    public static function generateProjectNumber(): string
    {
        ...
        $lastProject = self::withTrashed()
            ...
            ->orderByRaw('CAST(SUBSTRING_INDEX(project_number, "-", -1) AS UNSIGNED) DESC')
            ->first();
    ...
    {
        // Project value (what the customer owes) = budget incl. GST + add-ons.
    ...
    public function getTotalSpentAttribute(): float
    {
        ...
        // (expense_type='service', carrying project_id). Summing every expense's
    ...
    {
        $total = 0;
        foreach ($this->projectServices as $ps) {
            $total += $ps->balance;
        }
        return $total;
    }
    ...
    public function getPendingPaymentsSummary(): array
    {
        ...
        $pendingServices = $this->projectServices()
            ->get()
